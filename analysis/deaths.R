...
#Top 5 statesby death

#top deaths
COVID_totals %>%
  dplyr::select(date, positive, negative, death, state) %>% 
  group_by(state) %>% summarize(TotalDeath = max(death)) %>%
  arrange(desc(TotalDeath)) %>% as.data.frame()

#top positive cases
COVID_totals %>%
  dplyr::select(date, positive, negative, death, state) %>% 
  group_by(state) %>% summarize(TotalPositives = max(positive)) %>%
  arrange(desc(TotalPositives)) %>% as.data.frame()
...
COVID_totals %>%
  dplyr::select(date, positive, negative, death, state) %>% 
  group_by(state)%>%
  summarize(TotalPerc=max(positive)/(max(positive)+max(negative))) %>% 
 arrange(desc(TotalPerc)) %>% as.data.frame()
...
var(AR8) #651.9295
mean(deaths$new_death) #78.90987
...
length(deaths$new_death) #233
f_short = fore.aruma.wge(x = deaths$new_death, n.ahead = 7,lambda=c(rep(0,6), .7),
                         phi=difs.7_ests$phi)

dates = seq(as.Date(deaths$date[233]), by='day', length.out=8)[c(-1)]
...
dates = seq(as.Date(deaths$date[227]), by='day', length.out=7)
...
ASE_small_d = mean((deaths$new_death[227:233] - f_short$f)**2)
ASE_small_d  #973.1014 -> overpredicting deaths

labels = data.frame(x=c(zoo::as.Date('2020-05-1')), y=c(200), label=c(paste('ASE:', base::round(ASE_small_d,2))))
labels
...
dates = seq(as.Date(deaths$date[233]), by='day', length.out=94)[c(-1)]
...
ASE_long_d = mean((deaths$new_death[141:233] - f_long$f)**2)
ASE_long_d # 1401.118 -> overpredicting deaths

dates = seq(as.Date(deaths$date[141]), by='day', length.out=93)
...
labels = data.frame(x=c(zoo::as.Date('2020-05-1')), y=c(200), label=c(paste('ASE:', base::round(ASE_long_d,2))))
labels
...
var(AR8) #38148.78
mean(deaths_us$new_death) # 1021.07
ljung.wge(AR8) #FTR
...
dates = seq(as.Date(deaths_us$date[229]), by='day', length.out=8)[c(-1)]
...
ASE_short = mean((deaths_us$new_death[223:229] - short_f$f)**2)
ASE_short # 165388.1` `


dates = seq(as.Date(deaths_us$date[223]), by='day', length.out=7)
...
dates = seq(as.Date(deaths_us$date[229]), by='day', length.out=94)[c(-1)]
...
ASE_long_d = mean((deaths_us$new_death[137:229] - f_long$f)**2)
ASE_long_d #126604.5 -> overpredicting deaths

dates = seq(as.Date(deaths_us$date[137]), by='day', length.out=93)
