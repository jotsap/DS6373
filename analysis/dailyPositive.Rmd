...
We pick a training size of 48 and get a much higher Window ASE of 1,195,687,082 
1215342231
...
Forecasting out 90 days. As with a seasonal AR2 it cycles back towards the mean
...
boxplot(
  COVID_totals_final$pos_sum ~ COVID_totals_final$is_weekend, 
  horizontal = T,
  main = "Total Positive Cases: Weekdays vs Weekends")


boxplot(
  COVID_totals_final$perc_pos ~ COVID_totals_final$is_weekend, 
  horizontal = T,
  main = "Positivity Rate: Weekdays vs Weekends")


boxplot(
  COVID_totals_final$death_sum ~ COVID_totals_final$is_weekend, 
  horizontal = T,
  main = "Total Deaths: Weekdays vs Weekends")

boxplot(
  COVID_totals_final$perc_death ~ COVID_totals_final$is_weekend, 
  horizontal = T,
  main = "Mortality Rate: Weekdays vs Weekends")
...
# VAR model
VAR(new_pos.mtx, p=7, type = "trend") -> new_pos.var.train
# predictions
...
We overfit with p value of 12 and look at the factor table. When we overfit we see moderrate frequencies .28 
...
We pick a training size of 50 and get a better Window ASE of 12621120 
...
Forecasting out 90 days. As with $(1 - B)$2 AR12 it cycles back towards the last value
...
forecasts <- data.frame(
  preds = (new_pos_ca.fcast7$f + as.vector(new_pos.mlp.f7$mean))/2, 
  dates = dates
  )
...
# picks p of 9
VARselect(new_pos_ca.mtx, lag.max = 12, type = "both" )$selection[1]
...
VAR(new_pos_ca.mtx, p=9, type = "both") -> new_pos_ca.var
# predictions
predict(new_pos_ca.var, n.ahead = 30 ) -> new_pos_ca.var.pred30
...
forecasts <- data.frame(
  preds = new_pos_ca.var.pred30$fcst$y1[,1], 
  upper = new_pos_ca.var.pred30$fcst$y1[,3],
  lower = new_pos_ca.var.pred30$fcst$y1[,2], 
  dates = dates
  )
...
#combine 7 day forecasts with previous dependent vars
xreg_ca30 <- data.frame(
  is_weekend = ts(c(COVID_totals_final_ca$is_weekend, next30_isweekend)), 
  time = ts(c( t , next30time))
  )

#forecast the next 7 total positive datapoints using the 
#forecasted time and is_weekend as predictors
new_pos_ca.mlp.next30 <- forecast(new_pos_ca.mul.mlp, h = 30, xreg = xreg_ca30)
plot(new_pos_ca.mlp.next30)
...
forecasts <- data.frame(
  preds = (new_pos_ca.fcast7$f + as.vector(new_pos_ca.mlp.f$mean) + as.vector(new_pos_ca.mlp.next7[[2]]) + new_pos_ca.var.pred7$fcst$y1[,1] )/4, 
  dates = dates
  )
...
# MLP Univariate
lines(seq(1,7), as.vector(new_pos_ca.mlp.f$mean) , col = "blue")
# MLP Multivariate
...
forecasts <- data.frame(
  preds = (new_pos_ca.fcast30$f + as.vector(new_pos_ca.mlp.f$mean) + as.vector(new_pos_ca.mlp.next30[[2]]) + new_pos_ca.var.pred30$fcst$y1[,1] )/4, 
  dates = dates
  )
...
# ARIMA Univariate
plot(new_pos_ca.fcast30$f,type = "l")
# VAR Multivariate
lines(seq(1,7), as.vector(new_pos_ca.var.pred30$fcst$y1[,1]) , col = "red")
# MLP Univariate
lines(seq(1,7), as.vector(new_pos_ca.mlp.f30$mean) , col = "blue")
# MLP Multivariate
lines(seq(1,7), as.vector(new_pos_ca.mlp.next30[[2]]) , col = "green")
...
#### ALTERNATIVE: MEDIAN ENSEMBLE FOR LONG TERM FORECAST
...
forecasts <- data.frame(
  preds = median(nnew_pos_ca.fcast30$f + as.vector(new_pos_ca.mlp.f30$mean) + as.vector(new_pos_ca.mlp.next30[[2]]) + as.vector(new_pos_ca.var.pred30$fcst$y1[,1]) ), 
  dates = dates
  )
...
## Multiple Regression: CA State

**Explanatory Parameters*
* t: incremental time with 1 being April 1, 2020
* is_weekend: weekend classified as Saturday or Sunday

**Response Variable**
* new_pos: daily positive cases


Dates as time *t* is shown to be significant
Daily hospital is shown to be significant
Weekend is shown to *NOT* be significant :. we will exclude it

>Coefficients:
                     Estimate Std. Error t value Pr(>|t|)    
(Intercept)        -7.025e+04  5.900e+03 -11.906   <2e-16 ***
t                   4.112e+02  1.976e+01  20.815   <2e-16 ***
is_weekend Weekend -1.654e+03  3.017e+03  -0.548    0.584    
hospital_sum        1.490e+00  9.514e-02  15.663   <2e-16 *** 


Residuals do not appear white

ASE score is 441315118
...
#t <- 1:length(COVID_totals_final_ca$pos_sum)
t <- as.numeric(COVID_totals_final_ca$date - min(COVID_totals_final_ca$date) + 1 )
new_pos <- COVID_totals_final_ca$new_pos
is_weekend <- COVID_totals_final_ca$is_weekend
hospital_sum <- COVID_totals_final_ca$hospital_sum


# fit regression model
lm(new_pos ~ t + is_weekend + hospital_sum  ) -> new_pos.lm
summary(new_pos.lm)

#plot regression fit
plot(t, new_pos)
abline(lm(new_pos ~ t + hospital_sum + is_weekend), col = 'red' )

# plot residuals
plot(new_pos.lm)

# ASE
mean((new_pos.lm$residuals)^2)
...
Cannot run AIC on residuals. Shown as non-stationary
...
Error in aic calculation at 16 0 
Error in aic calculation at 17 0 
Error in aic calculation at 18 0 
Error in aic calculation at 19 0 
Error in aic calculation at 20 0 


```{r }

#graph residuals
plotts.sample.wge(new_pos.lm$residuals)

est.ar.wge( new_pos.lm$residuals , p=12, type = "burg")

#difference
new_pos.lm.d1 <- artrans.wge(new_pos.lm$residuals, phi.tr = 1)
aic5.wge( new_pos.lm.d1, p=0:20, q=0:0 )

#BIC GIVES ERRORS AT EVERY LAG
#aic5.wge( new_pos.lm.d1, p=0:20, q=0:0, type = "BIC" )
